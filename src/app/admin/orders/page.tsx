    ...
    return (
        <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-[#0F172A]">📋 Lịch sử hóa đơn</h1>
                    <p className="text-sm text-[#64748B] mt-1">Tất cả đơn hàng của nhà hàng</p>
                </div>
                <select
                    value={statusFilter}
                    onChange={e => { setStatusFilter(e.target.value); setPage(0) }}
                    className="border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-[#DC2626]"
                >
                ...
                <table className="w-full">
                    <thead className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                        <tr>
                            ...
                            <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748B] uppercase hidden sm:table-cell">Thời gian</th>
                            <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748B] uppercase hidden md:table-cell">Bàn / NV</th>
                    ...
                    <tbody className="divide-y divide-[#E2E8F0]">
                        {isLoading ? (
                            ...
                            <tr><td colSpan={5} className="text-center text-[#64748B] text-sm py-12">Không có đơn nào</td></tr>
                        ) : (
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            ...
                                            {order.payment_method && (
                                                <div className="text-xs text-[#64748B]">{order.payment_method === 'cash' ? '💵' : '🏦'}</div>
                                            )}
                                        </td>
                                    </tr>
                                )
                            ...
                            <div className="flex items-center gap-2">
                                {['completed', 'paid'].includes(selectedOrderDetail.status) && (
                                    <button
                                        ...
                                        className="px-3 py-1 bg-[#DC2626] text-white text-xs font-semibold rounded hover:bg-[#B91C1C] transition-colors"
                                    >
                                        In Bill
                                    </button>
                                )}
                                <button onClick={() => setSelectedOrder(null)} className="p-1.5 text-[#64748B] hover:text-[#0F172A]">✕</button>
                            </div>
                        </div>
                        <div className="p-5 space-y-4">
                            <div className="grid grid-cols-2 gap-3 text-sm">
                                ...
                                {selectedOrderDetail.source && (
                                    <div>
                                        <p className="text-[#64748B] text-xs">Nguồn</p>
                                    ...
                                    {selectedOrderDetail.order_items
                                        ...
                                        <div key={item.id} className="flex justify-between text-sm">
                                            <div>
                                                ...
                                                <span className="text-[#64748B] ml-2">x{item.quantity}</span>
                                                {item.note && <div className="text-xs text-[#64748B] italic">📝 {item.note}</div>}
                                            </div>
