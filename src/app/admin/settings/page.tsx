...
export default function SettingsPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-[#1A1A1A]">⚙️ Bàn & Cài đặt</h1>
                <p className="text-sm text-[#6B6B6B] mt-1">Quản lý bàn và mã QR</p>
            </div>

            {/* Table Management */}
            <div className="bg-white rounded-xl border border-[#E0DCD4] shadow-sm">
                <div className="flex items-center justify-between px-5 py-4 border-b border-[#E0DCD4]">
                    <h2 className="font-semibold text-[#1A1A1A]">Danh sách bàn</h2>
                    <button onClick={() => setShowAddTable(true)}
                        className="flex items-center gap-1.5 text-sm bg-[#D4553A] text-white px-3 py-2 rounded-lg hover:bg-[#B8432C] transition-colors">
                ...
                {isLoading ? (
                    <div className="p-5 space-y-3">
                        {[...Array(4)].map((_, i) => <div key={i} className="h-12 bg-[#F0EDE6] rounded-lg animate-pulse" />)}
                    </div>
                ) : (
                    <div className="divide-y divide-[#E0DCD4]">
                        {tables.map(table => (
                            <div key={table.id} className="flex items-center gap-4 px-5 py-3.5">
                                <div className="flex-1">
                                    <p className="text-sm font-medium text-[#1A1A1A]">{table.name || `Bàn ${table.table_number}`}</p>
                                    <p className="text-xs text-[#6B6B6B] font-mono mt-0.5 truncate max-w-xs">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="p-2 text-[#6B6B6B] hover:text-[#D4553A] hover:bg-[#FEF1ED] rounded-lg transition-colors"
                                        title="Xem QR"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="p-2 text-[#6B6B6B] hover:text-[#E8A317] rounded-lg transition-colors"
                                        title="Tạo lại QR"
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${table.is_active ? 'bg-green-100 text-green-700' : 'bg-[#F0EDE6] text-[#6B6B6B]'
                                            }`}
                                    >
                        ...
                        {tables.length === 0 && (
                            <p className="text-center text-[#6B6B6B] text-sm py-10">Chưa có bàn nào. Thêm bàn đầu tiên!</p>
                        )}
                    </div>
                )}
            </div>
            ...
            {showAddTable && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl p-6">
                        <h3 className="font-semibold text-[#1A1A1A] mb-4">Thêm bàn mới</h3>
                        <div className="space-y-3">
                            <div>
                                <label className="text-sm font-medium text-[#1A1A1A] block mb-1.5">Tên bàn *</label>
                            ...
                            <div>
                                <label className="text-sm font-medium text-[#1A1A1A] block mb-1.5">Số bàn</label>
                        ...
                        <div className="flex gap-3 mt-5">
                            <button onClick={() => setShowAddTable(false)}
                                className="flex-1 py-2.5 text-sm border border-[#E0DCD4] rounded-xl hover:bg-[#F0EDE6] text-[#6B6B6B]">Hủy</button>
                            <button onClick={() => addTable.mutate()} disabled={addTable.isPending}
                                className="flex-1 py-2.5 text-sm bg-[#D4553A] text-white rounded-xl hover:bg-[#B8432C] font-medium disabled:opacity-60">
            ...
            {qrPreview && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-xs shadow-xl p-6 text-center">
                        <h3 className="font-semibold text-[#1A1A1A] mb-4">{qrPreview.name}</h3>
                        ...
                        <p className="text-xs text-[#6B6B6B] mb-4 break-all font-mono">
                            {appUrl}/order/{qrPreview.token}
                        </p>
                        <button onClick={() => setQrPreview(null)}
                            className="w-full py-2.5 text-sm border border-[#E0DCD4] rounded-xl hover:bg-[#F0EDE6] text-[#6B6B6B]">Đóng</button>
                    </div>
                </div>
            )}
        </div>
