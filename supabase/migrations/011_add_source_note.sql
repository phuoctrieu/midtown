...
BEGIN
    ...
    IF p_action = 'accepted' THEN
        ...
        -- Format note
        v_note := '[Khách order]';
        IF v_request.note IS NOT NULL AND TRIM(v_request.note) <> '' THEN
            v_note := '[Khách order] ' || v_request.note;
        END IF;
        ...
        VALUES (v_order_id, v_request.menu_item_id, v_request.quantity, v_item_price, v_note)
        ON CONFLICT (order_id, menu_item_id)
        DO UPDATE SET 
            quantity = public.order_items.quantity + EXCLUDED.quantity,
            note = public.order_items.note || CASE WHEN public.order_items.note NOT LIKE '%[Khách order]%' THEN ' ' || v_note ELSE '' END;
